 ...
 * in a <em>host</em> object. The NineProperty's {@link #set(Object, Object)} or {@link #get(Object)}
 ...
 */
public abstract class NineProperty<T, V> {

    private final String mName;
    private final Class<V> mType;

    /**
     * This factory method creates and returns a NineProperty given the <code>class</code> and
     ...
     * <code>setName()</code> method is not found, the <code>NineProperty</code> will be
     ...
     * {@link NineNoSuchPropertyException} exception will be thrown.</p>
     */
    public static <T, V> NineProperty<T, V> of(Class<T> hostType, Class<V> valueType, String name) {
        return new NineReflectiveProperty<T, V>(hostType, valueType, name);
    }

    /**
     * A constructor that takes an identifying name and {@link #getType() type} for the property.
     */
    public NineProperty(Class<V> type, String name) {
        mName = name;
        mType = type;
    }
     ...
     * NineNoSuchPropertyException} exception). This may happen if the NineProperty wraps functionality that
     * allows querying the underlying value but not setting it. For example, the {@link #of(Class,
     * Class, String)} factory method may return a NineProperty with name "foo" for an object that has
