...
/**
 * Internal class to automatically generate a NineProperty for a given class/name pair, given the
 * specification of {@link NineProperty#of(Class, Class, String)}
 */
class NineReflectiveProperty<T, V> extends NineProperty<T, V> {
     ...
     * field results in throwing NineNoSuchPropertyException.
     *
     ...
     */
    public NineReflectiveProperty(Class<T> propertyHolder, Class<V> valueType, String name) {
                    ...
                    try {
                        ...
                        try {
                            ...
                            if (!typesMatch(valueType, fieldType)) {
                                throw new NineNoSuchPropertyException("Underlying type (" + fieldType + ") " +
                                        "does not match NineProperty type (" + valueType + ")");
                            }
                            return;
                        } catch (NoSuchFieldException e5) {
                            // no way to access property - throw appropriate exception
                            throw new NineNoSuchPropertyException("No accessor method or field found for"
        ...
        if (!typesMatch(valueType, getterType)) {
            throw new NineNoSuchPropertyException("Underlying type (" + getterType + ") " +
                    "does not match NineProperty type (" + valueType + ")");
        }
     ...
     * object matches the type of the NineProperty. The extra checks for primitive types are because
     * generics will force the NineProperty type to be a class, whereas the type of the underlying
    ...
    public void set(T object, V value) {
        if (mSetter != null) {
            ...
            throw new UnsupportedOperationException("NineProperty " + getName() +" is read-only");
        }
    }
    ...
    /**
     * Returns false if there is no setter or public field underlying this NineProperty.
     */
    @Override
