...
public class AnimationCloning extends Activity {
    ...
    public class MyAnimationView extends View implements NineValueAnimator.AnimatorUpdateListener {
        ...
        private void createAnimation() {
            if (animation == null) {
                NineObjectAnimator anim1 = NineObjectAnimator.ofFloat(balls.get(0), "y",
                        0f, getHeight() - balls.get(0).getHeight()).setDuration(500);
                NineObjectAnimator anim2 = anim1.clone();
                anim2.setTarget(balls.get(1));
                anim1.addUpdateListener(this);

                ShapeHolder ball2 = balls.get(2);
                NineObjectAnimator animDown = NineObjectAnimator.ofFloat(ball2, "y",
                        0f, getHeight() - ball2.getHeight()).setDuration(500);
                animDown.setInterpolator(new AccelerateInterpolator());
                NineObjectAnimator animUp = NineObjectAnimator.ofFloat(ball2, "y",
                        getHeight() - ball2.getHeight(), 0f).setDuration(500);
                animUp.setInterpolator(new DecelerateInterpolator());
                NineAnimatorSet s1 = new NineAnimatorSet();
                s1.playSequentially(animDown, animUp);
                animDown.addUpdateListener(this);
                animUp.addUpdateListener(this);
                NineAnimatorSet s2 = (NineAnimatorSet) s1.clone();
