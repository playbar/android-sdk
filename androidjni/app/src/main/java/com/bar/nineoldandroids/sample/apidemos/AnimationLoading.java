...
import com.bar.animation.NineAnimatorInflater;
...
/**
 * This application demonstrates loading NineAnimator objects from XML resources.
 */
public class AnimationLoading extends Activity {
    ...
    public class MyAnimationView extends View implements NineValueAnimator.AnimatorUpdateListener {
        ...
        private void createAnimation() {
            ...
            if (animation == null) {
                NineObjectAnimator anim = (NineObjectAnimator) NineAnimatorInflater.
                        loadAnimator(appContext, R.anim.object_animator);
                anim.addUpdateListener(this);
                anim.setTarget(balls.get(0));

                NineValueAnimator fader = (NineValueAnimator) NineAnimatorInflater.
                        loadAnimator(appContext, R.anim.animator);
                fader.addUpdateListener(new NineValueAnimator.AnimatorUpdateListener() {
                ...
                NineAnimatorSet seq =
                        (NineAnimatorSet) NineAnimatorInflater.loadAnimator(appContext,
                        R.anim.animator_set);
                seq.setTarget(balls.get(2));

                NineObjectAnimator colorizer = (NineObjectAnimator) NineAnimatorInflater.
                        loadAnimator(appContext, R.anim.color_animator);
                colorizer.setTarget(balls.get(3));

                animation = new NineAnimatorSet();
                ((NineAnimatorSet) animation).playTogether(anim, fader, seq, colorizer);
            }
        }
