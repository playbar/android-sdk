...
import com.bar.imagezoom.graphics.FastBitmapDrawable;
import com.bar.imagezoom.utils.IDisposable;
...
public abstract class ImageViewTouchBase extends ImageView implements IDisposable {
    ...
    private NineAnimator mCurrentAnimation;
    ...
    protected void scrollBy(float distanceX, float distanceY, final long durationMs) {
        final NineValueAnimator anim1 = NineValueAnimator.ofFloat(0, distanceX).setDuration(durationMs);
        final NineValueAnimator anim2 = NineValueAnimator.ofFloat(0, distanceY).setDuration(durationMs);

        stopAllAnimations();

        mCurrentAnimation = new NineAnimatorSet();
        ((NineAnimatorSet) mCurrentAnimation).playTogether(
            anim1, anim2
                                                      );
        ...
        anim2.addUpdateListener(
            new NineValueAnimator.AnimatorUpdateListener() {
                float oldValueX = 0;
                float oldValueY = 0;

                @Override
                public void onAnimationUpdate(final NineValueAnimator animation) {
        ...
        mCurrentAnimation.addListener(
            new NineAnimator.AnimatorListener() {
                @Override
                public void onAnimationStart(final NineAnimator animation) {

                }

                @Override
                public void onAnimationEnd(final NineAnimator animation) {
                ...
                @Override
                public void onAnimationCancel(final NineAnimator animation) {

                }

                @Override
                public void onAnimationRepeat(final NineAnimator animation) {
    ...
    protected void zoomTo(float scale, float centerX, float centerY, final long durationMs) {
        ...
        NineValueAnimator animation = NineValueAnimator.ofFloat(oldScale, finalScale);
